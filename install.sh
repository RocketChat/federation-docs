...
setup_matrix() {
	...
	if ((podman_compose)); then
		...
		[[ "${#@}" -gt 1 ]] && info "more than one ca cert detected, podman_compose would be broken" || true
		add_environment POSTGRES_PASSWORD "$postgres_password"
		add_environment CA_CERTIFICATE_PATH "$1"
	...
	podman_run "$matrix_container_name" "${volume_args[@]}" \
		-e UID="$(id -u)" -e GID="$(id -g)" \
		"$synapse_image"
}
